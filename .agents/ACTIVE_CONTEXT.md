...
* **Hitos Recientes Completados:**
  ...
  - **Ecommerce — UI de Vitrina y Dominios en el Admin + Despliegue Docker/Compose de la SPA (`StorefrontDomainsPage`, `storefrontApi`, `Dockerfile`, `nginx-spa.conf.template`, `docker-compose.yml`) [sin bump de versión]:**
    ...
    * **Verificación:** `tsc -b`/build del admin y E2E listada; build .NET limpio; imagen Docker construida y contenedor probado contra la API real: SPA 200 y `/api/.../storefront/products` devolviendo JSON con precio resuelto (proxy + Host correctos).
    * **Pendiente:** infraestructura Cloudflare for SaaS (CNAME + TLS por dominio) y carrito/checkout.
  - **Precios — Integración Tributaria y Bloqueo del Fallback (`BillingTaxRateProvider`, `CreateEcommerceOrderHandler`, migración `BackfillPricingFromBasePrice`) [sin bump de versión]:**
    * **Tarifas IVA:** `BillingTaxRateProvider` consulta `GET /api/v1/catalogs/tax-rates` de Facturación (`TaxCode` 2, `DefaultRateCode` 4) con caché por fecha (24 h), normaliza a fracción y cae a 15% si Facturación no responde; se configura en `BillingTaxRates` (`BaseUrl` vacío desactiva la consulta remota). 4 tests nuevos con handler HTTP falso.
    * **Fallback bloqueado:** `CreateEcommerceOrderItemInput/Request` ya no reciben `UnitPrice/DiscountAmount/TaxRate`; un pedido sin lista o precio vigente se rechaza con `ecommerce.order.price_not_configured` (test nuevo; el motor sigue siendo la única fuente del precio).
    * **Backfill de transición:** migración `BackfillPricingFromBasePrice` crea la lista `PUBLICO` por tenant y copia `BasePrice` a `product_prices` para raíces físicas activas (idempotente); aplicada en la BD local (29 listas, 16 precios) y vitrina verificada.
    * **Verificación:** build backend/frontend limpio, 327/327 Core y 229/229 Business (+5), smoke en `:5099` con migración y precios resueltos.
    * **Pendiente:** tarifa IVA por ítem (0/5/no objeto/exento) sigue como decisión de negocio; hoy la tarifa es única por tenant.
  - **Precios — Endurecimiento para Producción: IVA Incluido en Factura y Constraint de Solape (`pricingToLinePatch`, migración `AddProductPriceOverlapConstraint`) [sin bump de versión]:**
    * **Facturación IVA-incluido:** `pricingToLinePatch` (`invoiceFormTypes.ts`) desagrega la base imponible cuando `pricesIncludeTax = true` y expresa la promoción como descuento en base; el prellenado de línea y la re-resolución por cantidad lo consumen. Tests `scripts/pricing.invoice.test.ts` (`npm run test:pricing`, 5/5).
    * **Concurrencia:** migración `AddProductPriceOverlapConstraint` instala `btree_gist` y crea la restricción de exclusión `ex_product_prices_no_overlap` sobre `(tenant, lista, ítem, daterange[) )` para precios activos, con `RAISE WARNING` si falta privilegio; verificado en BD: dos rangos adyacentes insertan y el solape se rechaza.
    * **Decisión de negocio abierta (bloqueador de producción):** tarifas IVA reales por ítem (0/5/15/no objeto/exento) y confirmación de precios con/sin IVA; hoy `EcuadorTaxRateProvider` responde 15% fijo.
    * **Verificación:** 327/327 tests Core, 224/224 Business, `npm run build` y `npm run test:pricing` en verde; migración aplicada en la BD local dentro del smoke y datos temporales eliminados.
  - **Precios — Cierre de pendientes: orden por precio en vitrina, re-resolución por cantidad en factura y búsqueda por código de barras (`StorefrontCatalogRepository`, `useInvoiceEmitForm`, `ProductPriceRepository`) [sin bump de versión]:**
    * **Vitrina:** `StorefrontProductFilter` recibe la lista predeterminada y la fecha; `price_asc/price_desc` ordenan por precio resuelto (fallback a `BasePrice`): primero obtiene los ids ordenados y luego carga las entidades con imágenes/variantes para no perder los includes.
    * **Facturación:** cambiar la cantidad de una línea física re-resuelve escala/promoción/IVA con token anti-carrera y conserva el precio actual si el tenant no tiene pricing.
    * **Precios:** la búsqueda del listado encuentra por `custom_attributes_json.barcode`/`ean` (`EF.Functions.JsonContains`) además de nombre y SKU.
    * **Verificación:** 327/327 tests Core y 224/224 Business; `npm run build` limpio; smoke real contra la BD local con lista QA temporal: la vitrina ordenó por precio resuelto (5/12/20 y fallbacks 2.61/3.5/15.65), el predicado de código de barras coincidió y los datos QA se eliminaron.
  - **Precios — Fase 4: Integración con Ecommerce, Vitrina Pública y Prefill de Facturación (`CreateEcommerceOrderHandler`, snapshot `EcommerceOrderItem`, storefront, `useInvoiceEmitForm`) [sin bump de versión]:**
    * **Ecommerce server-side:** `CreateEcommerceOrderHandler` resuelve cada línea con `IPricingService` (escala, promoción e IVA según cantidad) y guarda snapshot en `ecommerce.order_items` (`price_list_id`, `list_price`, `applied_rules_json`); si el tenant no tiene lista/precio configurado cae al `UnitPrice` del request (compatibilidad) y, con pricing activo, el precio enviado por el cliente se ignora. Migración `AddEcommerceOrderItemPricingSnapshot`.
    * **Vitrina pública:** listado y detalle resuelven el precio desde la lista predeterminada (con herencia variante→padre en el detalle) vía `ListVigentByItemIdsAsync`; sin pricing configurado cae a `BasePrice`.
    * **Facturación:** la SPA prellena la línea al seleccionar un producto físico llamando a `resolve` (precio unitario, descuento e IVA) y conserva el precio base como respaldo; la validación fiscal de Billing sigue siendo la autoridad final.
    * **Verificación:** `dotnet build EcuNexo.slnx` limpio; 327/327 tests Core y 223/223 Business (+1 de ecommerce con motor); `tsc -b` y `npm run build` limpios; instancia temporal de la API en `:5099` aplicó `AddPricingModule` y `AddEcommerceOrderItemPricingSnapshot` y respondió el listado y detalle de la vitrina con resolución de precios.
    * **Pendiente:** completado en «Cierre de pendientes» (orden por precio en vitrina, re-resolución por cantidad y búsqueda por código de barras).
  - **Precios — Fase 3: UI de Gestión de Precios y Simulador en `ecunexo_admin` (páginas, servicio y E2E) [sin bump de versión]:**
    * **Rutas:** `catalogo/precios/{listas,productos,promociones,historial,simulador}` con vistas dedicadas de alta y edición (`/nueva`, `/:id`) registradas en `routes.tsx` y títulos en `getPageTitle.ts`.
    * **Pantallas:** listas de precios, precios por producto (búsqueda por nombre/SKU, filtros por lista/categoría/vigencia y baja lógica), promociones (tipo, valor, vigencia, prioridad, acumulabilidad y alcance por productos), historial de solo lectura con filtros y simulador que consume `POST /.../pricing/resolve` (mismo motor de ventas).
    * **Servicios y tipos:** `pricingApi.ts` + `types/pricingApi.ts` (15 operaciones), selector reutilizable `CatalogItemPicker` y utilidades de dinero/vigencia en `pricingFormat.ts`.
    * **Backend de apoyo:** `GET /catalog/pricing/prices/{id}` (detalle con escalas) y nombres de producto/lista en el historial (`PriceHistoryRow` + join en `PriceChangeLogRepository`).
    * **Verificación:** `tsc -b` y `npm run build` limpios; spec Playwright `tests-ui/comun/pricing-ui.spec.ts` con 3 casos listados; lint sin hallazgos nuevos salvo el patrón `set-state-in-effect` ya presente en los listados existentes.
    * **Pendiente:** Fase 4 (ecommerce server-side + snapshot y prefill de facturación) y búsqueda por código de barras en el listado de precios.
  - **Precios — Fase 2: Motor, CQRS, Endpoints y RBAC/Menú de Gestión de Precios (`PricingService`, `PricingEndpoints`, repositorios y seeders) [sin bump de versión]:**
    * **Motor:** `IPricingService`/`PricingService` resuelve lista (explícita o predeterminada), vigencia con herencia variante→padre matriz, promociones aplicables por alcance/fecha, escala por cantidad, descuento, IVA (`ITaxRateProvider` con `EcuadorTaxRateProvider` 15% MVP) y redondeo; devuelve `PricingResult` con `AppliedRules` (`LISTA_*`, `ESCALA_*`, `PROMO_*`, `PRECIO_HEREDADO_PADRE`).
    * **CQRS:** 9 comandos (listas, precios y promociones) y 5 queries (listados, historial y `resolve`); al crear un precio futuro se cierra la vigencia abierta anterior y cada cambio registra `PriceChangeLog`.
    * **API:** 14 endpoints bajo `/api/v1/tenants/{t}/catalog/pricing/...` protegidos con `catalog.pricing.read/create/update/delete/history.read` y `catalog.promotions.manage`.
    * **Datos:** 4 repositorios EF (`PriceListRepository`, `ProductPriceRepository`, `PromotionRepository`, `PriceChangeLogRepository`) con consulta única de resolución y filtros por búsqueda, lista, categoría y fecha.
    * **RBAC/Menú:** permisos y grupo «Gestión de precios» (Listas, Precios de productos, Promociones, Historial, Simulador) en `MenuCatalogSeedData.cs` y `navigation.v1.json`; las rutas frontend quedan para la Fase 3.
    * **Verificación:** `dotnet build EcuNexo.slnx` limpio; 327/327 tests Core y 220/220 tests Business (+17: motor, herencia, promociones, impuesto incluido, cierre de vigencia y conflictos).
    * **Pendiente:** Fase 3 (UI y simulador en `ecunexo_admin`), Fase 4 (ecommerce server-side + snapshot y prefill de facturación) y búsqueda por código de barras en el listado de precios.
  - **Precios — Fase 1: Contexto `Pricing`, Dominio Comercial y Migración `AddPricingModule` (ADR-012, `EcuNexo.Core/Pricing`, `EcuNexo.Data/Configurations/Pricing*`, `PriceCalculator`) [sin bump de versión]:**
    * **Taxonomía:** Pricing nace como capacidad del módulo raíz `catalog` (permisos canónicos `catalog.pricing.*` y `catalog.promotions.manage`), sin módulo satélite ni cambios de licenciamiento; contexto acotado propio con esquema PostgreSQL `pricing`.
    * **Dominio:** `PriceList` (predeterminada única por empresa vía índice único parcial, vigencias, moneda y `prices_include_tax`), `ProductPrice` (precio por lista/ítem con vigencia e historial inmutable), `QuantityTier` (escalas sin superposición), `Promotion` + `PromotionTarget` (tipos percentage/fixed_amount/fixed_price, prioridad, acumulable y alcance extensible) y `PriceChangeLog` append-only (quién, cuándo, precio anterior/nuevo, lista, ítem y vigencia).
    * **Motor (base):** `PriceCalculator` puro en Core con orden lista → escala → promoción (prioridad/acumulabilidad) → descuento → neto → impuesto (incluido o sumado) → final, redondeo `AwayFromZero` a 2 decimales y reglas aplicadas (`ESCALA_*`, `PROMO_*`).
    * **Datos:** 6 tablas en `pricing` con índices (unique `(tenant, code)`, lookup `(tenant, ítem, lista, vigencia)`, búsqueda de promociones) y migración `AddPricingModule`.
    * **Verificación:** `dotnet build EcuNexo.slnx` limpio; 327/327 tests Core (+39 de Pricing) y 203/203 tests Business; script SQL de la migración generado sin errores.
    * **Entregables de contexto:** ADR `012-pricing-module.md`; skills `pricing-listas-precios-vigencias`, `pricing-engine-resolucion-precios`, `pricing-ui-gestion-simulador`; plan `doc/plan-gestion-precios.md` (fases 0-5).
    * **Pendiente:** Fase 2 (handlers CQRS, `IPricingService`, endpoints, permisos/menú y seeders), Fase 3 (UI y simulador en `ecunexo_admin`), Fase 4 (ecommerce server-side + snapshot y prefill de facturación) y cierre de preguntas IVA Ecuador.
  - **Ecommerce — Vitrina pública multi-dominio (storefront por empresa) (`StorefrontEndpoints.cs`, `StorefrontAdminEndpoints.cs`, `StorefrontDomain`, migración `AddStorefrontDomains`, `ResolveStorefrontHandler`, SPA `Monorepo/ecommerce`, planes de `Licencias`) [sin bump de versión]:**
    * **Resolución por dominio:** `GET /api/v1/public/storefront/resolve?host=` (anónimo, cacheado 60 s con `IMemoryCache`) devuelve `tenantId` + branding (nombre, logo, color) y solo responde dominios verificados de tenants activos con entitlement `ecommerce`; `VITE_TENANT_ID` queda como override de desarrollo.
    * **Dominios por tenant:** tabla `tenancy.storefront_domains` (unique global, máx. 10 por empresa, token TXT `_ecunexo.<dominio>`), CRUD admin en `/tenants/{id}/ecommerce/storefront/domains` con verificación DNS TXT real (`DnsClient`) y permiso `ecommerce.storefront.manage`; el primer dominio queda principal y solo un dominio verificado puede serlo.
    * **API pública del catálogo:** `GET .../storefront/products` (paginado, búsqueda ILIKE, categoría con descendientes, orden; **solo productos físicos**, los servicios quedan excluidos), `.../products/{id}` (ficha sanitizada con herencia de imágenes y descriptor de matriz; un servicio responde `not_found`) y `.../categories`; tenant validado (no suspendido/cancelado), entitlement `ecommerce` y solo ítems activos.
    * **Disponibilidad:** `IStockRepository.SumAvailableByItemIdsAsync` suma `Quantity - ReservedQuantity` por ítem/variante; la vitrina expone `availableQuantity` e `inStock` sin exponer costo ni campos internos.
    * **Licenciamiento aplicado en `Monorepo/Licencias`:** `ecommerce` agregado a los planes Local/Empresa/Cadena/Grupo (seeder), tiers por plan para identidad/catálogo/bodegas/inventario/ecommerce y corrección de la consistencia `license.tier_consistency` (Catálogo > Identidad rompía la emisión); descripciones comerciales de la SPA actualizadas con la vitrina pública.
    * **SPA `Monorepo/ecommerce`:** bootstrap por `Host` con `storefrontSlice` (override por env), tenant dinámico en los thunks, branding (nombre, logo, `--store-accent`), axios con interceptores (`X-Request-Id`, tenant, contador de pendientes, ProblemDetails), Redux Toolkit, React Router 7 con lazy loading y CSS Modules.
    * **Verificación:** `dotnet build EcuNexo.slnx` limpio; 288/288 tests Core + 203/203 tests Business (22 nuevos de storefront/dominios); SPA ecommerce con 33/33 tests Vitest, ESLint y build de producción en verde.
    * **Pendiente:** página de dominios en `ecunexo_admin`, infraestructura Cloudflare for SaaS + wildcard + proxy same-origin, CORS de producción, carrito/checkout, Docker/nginx y bump SemVer de `Licencias` (1.12.0 → 1.13.0) si se libera.
  - **Facturación — Correo al Cliente con RIDE PDF y XML Adjuntos, Acciones de Grid en Cabecera y Ajustes de Emisión (`invoiceEmail.ts`, `invoiceEmitApi.ts`, `FacturasGrid.tsx`, `InvoiceEmailEndpoints.cs`, `SmtpEmailSender.cs`, `FacturaEmitirPage.tsx`, 31 listados) [v0.53.0]:**
    * **Correo con adjuntos:** al autorizar el SRI se envía al cliente el RIDE PDF (`@react-pdf/renderer`) y el XML firmado/autorizado como adjuntos; también en reenvíos al SRI que terminan autorizados y con acción manual «Reenviar correo al cliente» por fila en comprobantes y monitoreo SRI.
    * **Backend de correo:** `EmailMessage`/`IEmailSender` soportan adjuntos; `SmtpEmailSender` los envía por MailKit y `InvoiceEmailEndpoints` valida tipo PDF/XML, hasta 4 adjuntos, 4 MB c/u y 5 MB total (límite nginx 8m); tests nuevos `InvoiceEmailAttachmentTests` (8 casos).
    * **Acciones de grid en cabecera:** los botones de acción de las 31 pantallas de listado salen del `PageHeader` hacia `toolbarRight` del DataGrid, junto a filtros y refrescar.
    * **Ajustes UI:** cabecera de grids a `0.8125rem` con letter-spacing reducido; en emisión de factura «Solo validar» y «Solo firmar» quedan restringidos al ambiente de pruebas.
    * **Verificación:** `dotnet build EcuNexo.slnx`, 174+275 tests .NET, `tsc -b`, `npm run build`, 16/16 plantillas y 6/6 inventario en verde.
  - **Catálogo — Fix UX «Nuevo producto»: la plantilla manda en el formulario (`CreateCatalogItemPage.tsx`, `catalogo-ui.spec.ts`) [sin bump de versión]:**
    * **Problema:** con el modo «Usar una plantilla» activo y sin plantilla elegida, se mostraban igualmente «Datos del producto» y «Fotografías», permitiendo cargar datos genéricos que la plantilla luego reemplaza.
    * **Corrección:** `templateReady` bloquea los bloques de datos/fotos/variaciones hasta que se elija plantilla; se añadió la guía «Selecciona una plantilla para cargar el formulario»; el guard de `onSubmit` valida primero la plantilla y luego el nombre; al cambiar de modo se limpian atributos de plantilla heredados; eliminada la clase duplicada `ecu-section-page`.
    * **E2E:** `catalogo-ui.spec.ts` actualizado al flujo por tarjetas (servicio, un solo código y bucle de render/navegación).
    * **Verificación:** `tsc -b` y `pnpm build` limpios; capturas del estado sin plantilla, con plantilla y modo simple en `/tmp/opencode/create-*.png`.
  - **Barrido de Identidad de Sección en Todos los Módulos Restantes (57 vistas, `ecuSectionIdentity.css`, `GridToolbarRefresh`, grids y listas de Bodegas/Inventario/Compras/Facturación/Reparaciones/Clientes/Contabilidad/Organización/Dashboard) [sin bump de versión]:**
    * **Cobertura:** se añadió `ecu-section-page` a las 57 vistas con `ecu-dashboard-layout`; PageHeader, riel de recuento, `SectionCard` y DataGrid ledger quedan unificados en toda la app (Catálogo, Seguridad, Equipo y todos los módulos operativos).
    * **CSS seguro por defecto:** `.ecu-section-card__body` ahora usa `:has(.glb-datagrid)` para quitar padding solo cuando hay grid (los formularios conservan aire); el riel oculta iconos/footers/badges de forma automática.
    * **Acciones de grid:** se movió "Actualizar/Refrescar" al encabezado del DataGrid con `GridToolbarRefresh` en Bodegas, Inventario (Documentos/Stock/Kárdex), Compras (Documentos/Proveedores/Proformas/Tipos de Gasto/Liquidaciones), Facturación (Comprobantes/Notas/Guías/Monitoreo), Reparaciones (Lotes/Actas/Clientes/Portal) y Clientes/Organización.
    * **Celdas:** estados con `.ecu-status`, clasificaciones con `.ecu-chip`, textos largos con `.ecu-clip`, códigos con `.ecu-code`, fechas de grilla con `formatDate`.
    * **E2E alineado:** `identity-equipo.spec.ts` actualizado a los nuevos anclajes; `Módulo Compras` restaurado como badge; labels de Contabilidad y Plantillas restaurados para mantener specs en verde ("Total Asientos", "Volumen Contabilizado", "IVA Cobrado en Ventas", "SRI F104 & F103", "NIIF PYMES / SCVS Ecuador", "Total Plantillas", etc.).
    * **Limpieza:** eliminado el componente muerto `EcuModal` + su CSS; purgado CSS muerto de precio/categoría del catálogo (tras el desacoplamiento); ESLint bajó de 203 a 196 problemas (0 nuevos).
    * **Verificación:** `tsc -b`, `pnpm build` y capturas claro/oscuro/móvil de muestras por módulo en `/tmp/opencode` (`qa-*.png`).
  - **Equipo — Identidad de Sección y Acciones de Grid en Encabezado (`UsersListPage/Grid`, `RolesListPage/Grid`, `DepartmentsListPage/Grid`, `UserDetailPage`, `RoleDetailPage`, `RolePermissionsPage/Grid`, `EffectivePermissionsGrid`, formularios del módulo, `ecuSectionIdentity.css`) [sin bump de versión]:**
    * **Listas rediseñadas:** Usuarios/Roles/Departamentos con portada, riel de recuento (Usuarios/Activos/Sin acceso/Con acceso; Roles/Del sistema/Personalizados; Áreas/Con detalle), grid ledger y errores como banner dentro del `SectionCard`.
    * **Acciones de grid reubicadas:** "Actualizar" sale del `PageHeader` y se integra al encabezado del grid con `GridToolbarRefresh` en las tres listas; la navegación cruzada (Roles/Usuarios/Permisos) queda arriba.
    * **Fichas de detalle:** `UserDetailPage` y `RoleDetailPage` reemplazan las tarjetas KPI por tiles de propiedad (estado con marca, departamento, cargo, último acceso, alta; origen con chip, permisos, actualización) y eliminan acciones duplicadas del menú.
    * **Permisos del rol:** `RolePermissionsPage` con riel Catálogo/Asignados/En pantalla, matriz seleccionable con código monoespaciado y chips de módulo; `EffectivePermissionsGrid` unificado.
    * **Formularios:** los formularios del módulo (crear/editar usuario, rol y departamento; asignar rol; reset password) adoptan `ecu-section-page`, `SectionCard` con cuerpo padded, títulos planos sin iconos y sin subtítulos tutoriales, y banner de error unificado.
    * **Infra transversal:** `.ecu-perm-code` movido a la identidad compartida; `.ecu-form-error-banner` estilizado por fin (antes era una clase huérfana sin CSS) con variante dark; `.ecu-chip` admite enlaces.
    * **Verificación:** `tsc -b` y `pnpm build` limpios; ESLint sin hallazgos nuevos; capturas claro/oscuro/móvil de listas, fichas, permisos y formularios en `/tmp/opencode`.
  - **Catálogo — Desacoplamiento de Precio Base y Categoría en Ficha e Ítems (`CreateCatalogItemPage.tsx`, `EditCatalogItemPage.tsx`, `CatalogItemsGrid.tsx`) [sin bump de versión]:**
    * **Desacoplamiento funcional:** las categorías ahora son modeladas como atributos dinámicos/arquetipos en el diccionario de atributos (`customAttributesJson` / `VariantDimensionTemplate`), y los precios se gestionan a nivel de variante física o SKU comercial individual en lugar de un precio base arbitrario en el producto matriz.
    * **Limpieza de Formularios (`CreateCatalogItemPage.tsx` y `EditCatalogItemPage.tsx`):** se eliminaron los campos `ci-cat` / `ei-cat` (Categoría) y `ci-price` / `ei-price` (Precio base). Las mutaciones de alta (`createCatalogItemMatrix`, `createCatalogItem`) y edición (`updateCatalogItem`) envían `basePrice: null` y `categoryId: null` de forma agnóstica.
    * **Diseño Compacto y Agnóstico (Regla 11 & `ui-compacta-sin-tutoriales`):** grillas rebalanceadas en 3 y 4 columnas respectivamente (`Nombre`, `Código SKU`, `Descripción`), eliminando espacios vacíos; placeholders reemplazados por el estándar agnóstico `"Escriba aquí..."`.
    * **DataGrid y Tarjetas Móviles (`CatalogItemsGrid.tsx`):** se eliminaron las columnas `Categoría` y `Precio base` de la grilla de escritorio, así como el badge de categoría y la caja de precio en la vista móvil de tarjetas (`ecu-catalog-card__price-box`). Purga de código muerto (`CatalogPrice`).
    * **Pruebas y Verificación:** `npm run build` frontend limpio sin errores de tipo, 16/16 tests de plantillas, 6/6 tests de inventario y 441/441 tests backend .NET en verde.
  - **UI — Tipografía Única Poppins, Identidad de Sección Compartida y Módulo Seguridad (`ecuSectionIdentity.css`, `catalogGrid.css`, `GridToolbarRefresh.tsx`, `PermissionsCatalogPage.tsx`, `PermissionsGrid.tsx`, `PermissionDetailPage.tsx`, `PermissionPoliciesSection.tsx`, `PoliciesGrid.tsx`, `CreatePermissionPage.tsx`, `securitySection.css`, páginas de Catálogo, `index.html`, `index.css`) [sin bump de versión]:**
    * **Tipografía única:** se elimina Inter y la display Archivo Narrow; **Poppins** (400/500/600/700) queda como única familia de toda la app vía `--glb-font-family`, unificando también los componentes glubox/Sidebar.
    * **Identidad de sección compartida (`src/styles/ecuSectionIdentity.css`):** el sistema del Catálogo (portada, riel de recuento, `SectionCard` aplanada, DataGrid ledger, marcas `.ecu-status` / `.ecu-chip` / `.ecu-token` / `.ecu-price` / `.ecu-level`, modo oscuro y responsive) se generaliza bajo `.ecu-section-page` con tokens `--idt-*` y se reutiliza en Seguridad; `catalogSection.css` desaparece y `catalogGrid.css` conserva solo lo específico del catálogo (tarjetas móviles, miniatura).
    * **Módulo Seguridad rediseñado:** `/seguridad/permisos` con riel Permisos / Activos / Módulos, código monoespaciado como ancla de fila, chips de módulo y estados con punto; ficha del permiso con tiles de propiedad (código técnico, módulo, orden, estado, alta, descripción) y sección de políticas ABAC compactada; alta de permiso con placeholders agnósticos.
    * **Acciones de grid reubicadas:** "Actualizar/Refrescar" sale del `PageHeader` y pasa al encabezado del grid con el nuevo componente `GridToolbarRefresh` (Ítems, Categorías, Atributos, Plantillas y Catálogo de Permisos). En Seguridad, "Nuevo Permiso" queda como botón primario y "Roles" como navegación.
    * **Limpieza:** eliminada la doble carga de datos en `PermissionsCatalogPage`, párrafos tutoriales del formulario ABAC y subtítulos redundantes.
    * **Verificación:** `tsc -b` y `pnpm build` limpios; capturas claro/oscuro/móvil de Seguridad y Catálogo en `/tmp/opencode` (referencia, no versionadas).
  - **Catálogo — Identidad Visual de Sección, primera pasada (páginas y grids del módulo, `catalogGrid.css`) [sin bump de versión]:**
    * **Riel de recuento:** las 4 tarjetas KPI sueltas se reemplazan por una banda conectada con divisores hairline y numerales grandes; un solo acento (`--shell-primary` del tenant), sin colores decorativos hardcodeados.
    * **Codificación estructural y ledger:** físico = bloque sólido, servicio = contorno discontinuo; estado = punto + palabra; precio con símbolo y decimales atenuados; DataGrid con superficies planas, reglas internas y columna fija opaca.
    * **Limpieza:** búsquedas duplicadas en Atributos/Plantillas (`showSearch={false}`), fechas de alta compactas (`formatDate`), recorte con ellipsis y subtítulos redundantes retirados.
    * **Nota:** este hito queda generalizado por «Identidad de Sección Compartida» (arriba), que lo traslada a `.ecu-section-page` y a la tipografía única Poppins.
  - **Tributario & SRI — Skill Guías de Remisión Electrónicas (Tipo 06) (`.agents/skills/guias-remision-sri-ecuador/SKILL.md`) [sin bump de versión]:**
    * **Creación de la Skill `guias-remision-sri-ecuador`:** estándar integral para el comprobante complementario 06 conforme a la Ficha Técnica SRI v2.32 / XSD v1.1.0.
    * **Contenido Estandarizado:** reglas para transportistas (RUC/cédula, placa vehicular regex `^[A-Z0-9-]{3,20}$`), cálculo de fechas de inicio y fin de traslado, motivos de traslado oficiales (venta, traspaso de bodega, consignación, importación DAI), documentos de sustento vinculados (Factura 01), Clave de Acceso Módulo 11 (49 dígitos), ciclo de vida logístico (`Draft` → `Issued` → `Authorized` → `InTransit` → `Delivered`), integración con Traspasos de Bodega / Kárdex y matriz de validaciones anti-rechazo del WebService del SRI.
  - **Estándar UI & Skill — UI Compacta, Ergonómica y Agnóstica (Anti-Tutorial) (`.agents/skills/ui-compacta-sin-tutoriales/SKILL.md`, `GEMINI.md`, `CatalogAttributesListPage.tsx`) [sin bump de versión]:**
    * **Creación de la Skill `ui-compacta-sin-tutoriales`:** estándar exhaustivo de alta densidad visual enterprise y erradicación de antipatrones tipo tutorial/manual de usuario dentro de pantallas operativas.
    * **Regla 11 en `GEMINI.md`:** directriz obligatoria para mantener interfaces limpias, placeholders agnósticos (`Escriba aquí...`, `Seleccione uno...`, `Buscar...`), eliminación de párrafos explicativos en formularios y empty states densos no sermoneadores.
    * **Refactor Inmediato en Atributos (`CatalogAttributesListPage.tsx`):** modal compactado de 560px a 480px; eliminación de párrafos pedagógicos y bloques vacíos explicativos redundantes; sección de opciones solo visible condicionalmente para escalas con opciones; placeholders simplificados a términos neutros.
    * **Verificación:** 441/441 tests backend .NET, 16/16 tests de plantillas, 6/6 tests de inventario y build Vite frontend limpio.
  - **Catálogo — Simplificación Integral de UX en Creación de Atributos («Tipo de Atributo») (`CatalogAttributesListPage.tsx`, `HierarchyTemplateTreeBuilder.tsx`) [sin bump de versión]:**
    * **Unificación de Selectores Técnicos en «Tipo de Atributo»:** se eliminaron los 3 desplegables confusos y redundantes del formulario de alta/edición de atributos (*Tipo / Clasificación*, *Tipo de dato*, y *Uso en variantes: solo descriptivo vs genera variantes con SKU*).
    * **Tipos Naturales de Negocio:** reemplazados por un único campo desplegable **«Tipo de Atributo»** con opciones de lenguaje comercial comprensibles:
      - *Texto Libre / Descripción:* para notas, composición, observaciones o ficha técnica (no exige chips).
      - *Escala de Tallas o Medidas:* para variantes con opciones de tallas (exige opciones predefinidas).
      - *Muestras de Color:* para variantes cromáticas.
      - *Opciones de Variante:* para dimensiones físicas personalizadas (ej. Caña, Calibre).
      - *Número o Medida técnica con unidad:* despliega automáticamente el campo de unidad (`kg`, `cm`, `W`).
      - *Sí / No (Interruptor):* para características binarias.
      - *Selección múltiple:* etiquetas informativas sin multiplicar variantes.
    * **Mapeo Automático de Dominio:** el componente mapea internamente `dimensionType`, `dataType` y `isVariantAxis` al DTO de la API sin exponer conceptos técnicos de base de datos ni términos de programador al usuario.
    * **Grilla y Filtros Renovados:** la columna ahora muestra el badge del tipo claro (*Tallas / Medidas*, *Color*, *Opciones de Variante*, *Texto Libre*, *Número*, etc.) y el filtro superior permite segmentar por estos tipos reales.
    * **Ergonomía en el Árbol de Plantillas:** los distintivos en `HierarchyTemplateTreeBuilder.tsx` se actualizaron de `Descriptivo` a `Dato / Texto`, con mensajes orientados a la operación comercial.
    * **Pruebas y Verificación:** build frontend limpio, 16/16 tests de plantillas, 6/6 tests de inventario y 441/441 tests backend .NET en verde.
  - **Catálogo — Control y Blindaje Arquitectónico entre Ejes Físicos vs. Atributos Descriptivos (`HierarchyTemplateTreeBuilder.tsx`, `CatalogAttributesListPage.tsx`, `VariantDimensionTemplate.cs`, `CreateVariantDimensionTemplateValidator.cs`, `UpdateVariantDimensionTemplateValidator.cs`) [sin bump de versión]:**
    * **Restricción Estricta de Ejes en Árbol de Plantillas (`HierarchyTemplateTreeBuilder.tsx`):** el selector «Agregar del diccionario» en la sección *Ejes de este nivel* filtra y excluye todos los atributos descriptivos (`isVariantAxis === false`); en la lista de *Datos de este nivel*, los atributos descriptivos muestran la insignia `Descriptivo` en lugar del botón `Pasar a ejes`; si un usuario intenta ingresar manualmente un atributo descriptivo como eje, el sistema bloquea la acción con un toast explicativo.
    * **Invariante en Creación de Atributos (`CatalogAttributesListPage.tsx`):** por defecto los nuevos atributos se configuran como `Solo descriptivo del producto`; si el usuario selecciona `Genera variantes con SKU (eje físico)`, es obligatorio ingresar al menos una opción predefinida (chips); se impide registrar ejes sin valores discretos.
    * **Invariantes de Dominio y Backend (`VariantDimensionTemplate.cs`, validadores):** `NormalizeValuesJson` exige opciones discretas cuando `isVariantAxis: true` (`values.Count > 0`); y permite campos abiertos `"[]"` únicamente cuando `isVariantAxis: false` (descriptivos).
    * **Pruebas Automatizadas:** 441/441 tests backend .NET en verde, 16/16 tests frontend en `templates.formation.test.ts` y build Vite frontend limpio.
  - **Catálogo — Atributos de Solo Texto Libre / Abierto y Captura Descriptiva en Variantes (`VariantDimensionTemplate.cs`, `CatalogAttributesListPage.tsx`, `catalogArchetype.ts`, `VariantMatrixBuilder.tsx`, `templates.formation.test.ts`, `VariantDimensionTemplateTests.cs`) [sin bump de versión]:**
    * **Creación y Edición de Atributos de Texto Abierto (`CatalogAttributesListPage.tsx`):** se permite registrar y editar atributos con `dataType: 'text'` sin obligar la especificación de opciones predefinidas tipo chip (`formValues.length === 0`). El modal guía al usuario indicando que las opciones son opcionales y que el campo se comportará como texto abierto (ej. descripción, notas, composición, observaciones).
    * **Visualización en Grilla de Atributos:** cuando un atributo no tiene valores predefinidos, la columna correspondiente muestra el badge elegante `<StatusBadge tone="neutral">Texto libre / Abierto</StatusBadge>`.
    * **Flexibilidad en Dominio y Validadores Backend (`VariantDimensionTemplate.cs`, `CreateVariantDimensionTemplateValidator.cs`, `UpdateVariantDimensionTemplateValidator.cs`):** `NormalizeValuesJson` permite valores vacíos o arreglos `"[]"` cuando `dataType == DataTypeText`, preservando la regla de obligatoriedad únicamente para escalas discretas o físicas.
    * **Enrutamiento Arquetípico y Constructor de Variantes (`catalogArchetype.ts`, `VariantMatrixBuilder.tsx`):** `getVariantAttributeFields` ahora extrae los atributos descriptivos del nivel terminal incluso cuando la plantilla declara ejes (`templateDeclaresAxes`), permitiendo que cada fila de variante en `VariantMatrixBuilder` presente un campo `<TextBox>` para ingresar descripciones o notas particulares por cada SKU, guardándose en `customAttributesJson`.
    * **Pruebas Automatizadas:** 440/440 tests backend en verde (+1 test de dominio para atributos de texto libre), 16/16 tests frontend en `templates.formation.test.ts` (+1 test de captura de texto abierto en variantes) y build Vite frontend exitoso sin errores.
  - **Inventario — Selector Interactivo de Variantes, Recepción Matricial (*Matrix Ingress*) y Escáner de Códigos (`InventoryItemSelectModal.tsx`, `CreateInventoryDocumentPage.tsx`, `inventoryItemSelectModal.css`, `inventory.ingress.test.ts`) [sin bump de versión]:**
    * **Selector Modal Multidimensional (`InventoryItemSelectModal.tsx`):** selector modal con 2 pestañas para la entrada de existencias:
      - *Búsqueda de SKUs / Variantes:* búsqueda en vivo por SKU, nombre, modelo, color, talla o categoría; desglose de variación con `formatVariantDisplayName`; indicador visual de stock actual en la bodega seleccionada; inputs numéricos de cantidad e inserción por lotes.
      - *Recepción por Modelo / Matriz (Matrix Ingress):* selección del producto matriz padre, resumen del arquetipo/familia, despliegue de todas las variantes hijas en cuadrícula limpia con stock actual en bodega, barra de asignación masiva («Fijar cantidad común en todas») y carga en 1 clic al documento.
    * **Escáner Rápido de Barras y SKU en Alta de Documento (`CreateInventoryDocumentPage.tsx`):** barra de lectura en tiempo real sobre la tabla de líneas; captura pistola de código de barras o ingreso de SKU por teclado (`Enter`), buscando coincidencias directas o en `customAttributesJson.barcode`/`ean`; añade automáticamente la línea o incrementa la cantidad si ya existía.
    * **Acceso Contextual en Grilla:** botón interactivo con ícono `Boxes` en cada fila de la tabla para abrir el catálogo y sustituir o seleccionar la variante físicamente.
    * **Pruebas y Verificación:** 6/6 tests en `scripts/inventory.ingress.test.ts` (merge de líneas, reemplazo de línea vacía, match por SKU exacto/insensible a mayúsculas, exclusión de padres matriz y resolución de códigos de barras); 439/439 tests backend .NET en verde y build Vite frontend limpio.
