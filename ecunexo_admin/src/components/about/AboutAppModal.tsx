      ...
      <div className="ecu-about-modal">
        ...
        <div className="ecu-about-modal__changelog">
          <div className="ecu-about-modal__changelog-header">
            ...
            <span>Novedades v{APP_VERSION_INFO.version} — Catálogo Basado en Plantillas y Fotografía Exclusiva en Variantes</span>
          </div>
          <ul className="ecu-about-modal__changelog-list">
            <li><strong>Catálogo Basado en Plantillas y Fotografías Exclusivas en Variantes [v0.40.2]:</strong> Al elegir una plantilla en el alta de productos (<code style={{ fontSize: '0.8rem' }}>CreateCatalogItemPage</code>), el formulario se simplifica al 100%: se eliminan los campos redundantes de Categoría, Nombre manual, Precio base y Descripción, listando directamente las dimensiones intermedias del modelo en la tarjeta principal mediante selectores (<code style={{ fontSize: '0.8rem' }}>&lt;Select&gt;</code>) asistidos. Se suprime la zona de fotografías a nivel general cuando el producto tiene variantes, asignando las fotos exclusivamente a las variantes físicas (<code style={{ fontSize: '0.8rem' }}>VariantMatrixBuilder</code>) con zona de carga directa desde equipo (individual y en lote) y eliminación de textos de vitrina principal.</li>
            <li><strong>Consolidación de Atributos del Modelo, Galería Única de Fotos y Matriz Física de Variantes [v0.40.1]:</strong> Optimización de la experiencia de alta de catálogo (<code style={{ fontSize: '0.8rem' }}>CreateCatalogItemPage</code>). Consolidación de todos los atributos intermedios de la jerarquía (ej. Caña, Actividad, Disciplina, Material) en una sola tarjeta clara con menús desplegables (<code style={{ fontSize: '0.8rem' }}>&lt;Select&gt;</code>) prealimentados automáticamente con opciones y sinónimos del Diccionario Maestro. Eliminación total de zonas de carga de fotos repetidas por cada nivel, centralizándolas en una única galería de presentación del producto disponible para vincular a variantes. Delimitación estricta de la matriz terminal (<code style={{ fontSize: '0.8rem' }}>VariantMatrixBuilder</code>) para combinaciones físicas de inventario (Talla × Color) ubicadas al pie del formulario con generación de SKUs descriptivos.</li>
            <li><strong>Flujo Jerárquico Nivel por Nivel, Selección Asistida de Atributos y Fotos por Escala [v0.40.0]:</strong> Reestructuración arquitectónica integral del alta de productos (<code style={{ fontSize: '0.8rem' }}>CreateCatalogItemPage</code>). Renderizado ordenado nivel por nivel conforme al arquetipo de plantilla (Colección ➔ Modelo ➔ Variantes Físicas al final). Transformación de los atributos de niveles superiores a menús desplegables (<code style={{ fontSize: '0.8rem' }}>&lt;Select&gt;</code>) alimentados automáticamente del Diccionario Maestro de Atributos. Integración de la zona de fotografías directamente dentro del nivel jerárquico al que pertenecen (eliminando tarjetas flotantes de vitrina y banners interiores duplicados). Supresión de la casilla redundante de variantes en productos con plantilla e inicialización automática de dimensiones terminales y SKUs descriptivos por atributo (<code style={{ fontSize: '0.8rem' }}>skuFormat = 'name'</code>).</li>
            <li><strong>Columnas por Dimensión, Múltiples Fotos por Variante y Optimización de SKU [v0.39.1]:</strong> Desglose dinámico de cada dimensión activa (Caña, Talla, Color, etc.) en columnas independientes en la tabla del constructor de variantes (<code style={{ fontSize: '0.8rem' }}>VariantMatrixBuilder</code>) para máxima claridad visual. Soporte integral para múltiples fotografías por variante física (SKU) con carga de múltiples archivos desde equipo, previsualización con badge numérico (<code style={{ fontSize: '0.8rem' }}>+N</code>), gestión modal interactiva y carga en lote al backend. Generación inicial limpia de 1 sola variante y supresión del campo SKU en el producto padre al definir variantes.</li>
            <li><strong>Fotografías Individuales por Variante, Asignación Masiva y Arquitectura de Catálogo [v0.39.0]:</strong> Incorporación de asignación visual de fotografías independientes por variante física en el constructor de matriz (<code style={{ fontSize: '0.8rem' }}>VariantMatrixBuilder</code>) con mini-ranuras interactivas de previsualización, selector modal conectado a la galería principal del producto y herramienta de asignación fotográfica en lote en 1 clic filtrada por atributo (ej. aplicar foto a todas las tallas de una misma opción o color). Retiro definitivo de la sección de Categorías del menú de navegación y de la base de datos (<code style={{ fontSize: '0.8rem' }}>RetiredMenuItemIds</code>) consolidando el catálogo en Ítems, Atributos y Plantillas. Estandarización de placeholders dinámicos y genéricos en todo el catálogo adaptables automáticamente a cualquier sector comercial.</li>
            <li><strong>Ergonomía de Selección Instantánea y Modernización Visual en Plantillas [v0.38.1]:</strong> Optimización en el constructor de jerarquías (<code style={{ fontSize: '0.8rem' }}>HierarchyTemplateTreeBuilder</code>) con adición inmediata de atributos corporativos al elegirlos en el menú desplegable (eliminando la necesidad de un segundo clic sobre el botón <code style={{ fontSize: '0.8rem' }}>+</code> y reiniciando el selector para inserción ágil continua). Sustitución del contenedor gris opaco por superficies suaves M3 Enterprise SaaS (<code style={{ fontSize: '0.8rem' }}>color-mix</code>), bordes de alta definición y etiquetas luminosas para atributos activos y sugerencias en 1 clic. Eliminación de presets precargados fijos para permitir la creación libre y limpia de plantillas desde cero.</li>
            <li><strong>Reasignación Flexible de Variantes con Auditoría Inmutable [v0.38.0]:</strong> Nueva funcionalidad para mover y reasignar variantes hijas entre productos matriz o independizarlas como productos individuales sin alterar su SKU físico, facturación previa ante el SRI ni el stock real en bodega (unidades físicas y kárdex preservados). Registro inmutable de trazabilidad en JSONB (<code style={{ fontSize: '0.8rem' }}>parent_reassignment_history</code>) con motivo de auditoría obligatorio, usuario y fecha UTC. Banner contextual con acción rápida en ficha de variante, botón de reasignación directa en la grilla de variantes del padre, y tarjeta de línea de tiempo con historial completo de movimientos.</li>
