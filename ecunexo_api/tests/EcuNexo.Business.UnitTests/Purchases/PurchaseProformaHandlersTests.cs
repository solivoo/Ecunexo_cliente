...
public sealed class PurchaseProformaHandlersTests
{
    ...
    public async Task Handle_CreateProforma_CalculatesTotalsAndSaves()
    {
        ...
        var supplier = Supplier.Create(supplierId, tenantId, "Proveedor ABC", "1790016919001").Value!;
    ...
    [Fact(DisplayName = "ApprovePurchaseProformaHandler aprueba una proforma en borrador")]
    public async Task Handle_ApproveProforma_DraftStatus_ApprovesSuccessfully()
    {
        // Arrange
        var tenantId = Guid.NewGuid();
        var proformaId = Guid.NewGuid();
        ...
        var handler = new ApprovePurchaseProformaHandler(_proformas, _unitOfWork);
